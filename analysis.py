...
# Pastikan semua variabel yang diperlukan sudah tersedia
if all(var in locals() for var in ['data', 'x_items', 'y_items', 'create_total', 'x_norm', 'y_norm', 'r', 'p', 'strength', 'direction', 'method']):
    # Buat ulang variables_to_analyze untuk PDF
    variables_to_analyze_pdf = []
    if x_items:
        variables_to_analyze_pdf.extend(x_items)
    if y_items:
        variables_to_analyze_pdf.extend(y_items)
    if create_total:
        if "X_total" in data.columns:
            variables_to_analyze_pdf.append("X_total")
        if "Y_total" in data.columns:
            variables_to_analyze_pdf.append("Y_total")
    ...
    try:
        ...
        for idx, col in enumerate(variables_to_analyze_pdf):
            if col not in data.columns:
                continue
            ...
            if idx < len(variables_to_analyze_pdf) - 1:
                story.append(Spacer(1, 0.15*inch))
        ...
        if st.session_state.language == 'en':
            findings_list = f"""
            <b>1. Descriptive Analysis:</b> Successfully analyzed {len(variables_to_analyze_pdf)} variables, revealing meaningful patterns 
            ...
            findings_list = f"""
            <b>1. Analisis Deskriptif:</b> Berhasil menganalisis {len(variables_to_analyze_pdf)} variabel, mengungkapkan pola bermakna 
    ...
    # Tampilkan pesan warning dalam bahasa yang sesuai
    if st.session_state.language == 'en':
        st.warning("⚠️ Please run the analysis first to generate a PDF report.")
    else:
        st.warning("⚠️ Silakan jalankan analisis terlebih dahulu untuk menghasilkan laporan PDF.")
